...
To run the demo unzip the downloaded file (Ultibo-Demo-1.0.9.zip), copy the contents to the root directory of an SD card formatted with a FAT partition and use the card to boot your Raspberry Pi. 

You can also use an existing SD card created for Raspberry Pi OS (Raspbian) to run the demo by copying the kernel.img, kernel7.img, kernel7l.img, armstub32-rpi3.bin, armstub32-rpi4.bin and config.txt files from the downloaded zip to the root directory of the FAT partition.
...
Select Run, Build from the menu to build either KERNEL.IMG, KERNEL7.IMG or KERNEL7L.IMG depending on which project you opened above.
